#the code in ablation study CDAT_wo_Dpsi 
...
def Trainer(SRawData, TRawData, STrainData, TTrainData, TValidateData, TTestData, TValidateEvalData100, TTestEvalData100):
    ...
    OptimizerDFi = torch.optim.Adam(Net.Dan.D_Fi.parameters(), lr=SetParam.DFi_lr, weight_decay=SetParam.wd)  #Domain discriminator D_phi
    ...
    #OptimizerDPe = torch.optim.Adam(Net.Cdan.D_Pe.parameters(), lr=SetParam.DPe_lr, weight_decay=SetParam.wd) #remove in CDAT_wo_Dpsi
    ...
    for e in range(SetParam.epoch):
        ...
        for step, (SInterBatch, TInterBatch) in enumerate(zip(STrainDataLoad, cycle(TTrainDataLoad))):
            ...
            SUserEmbed, TUserEmbed, SItemEmbed, TItemEmbed = Net.R2Lfw()  #get initial embeddings
            SUserPosBatch = SUserEmbed[SInterBatch[0]].to(device=device)   #lookup
            TUserPosBatch = TUserEmbed[TInterBatch[0]].to(device=device)   #lookup
            ...
            SShareUserDIBatch = []  #auxiliary domain
            TShareUserDIBatch = []  #target domain
            ...
            # update Dpe 1st
            #DI, CDAE, SCDAE, TCDAE = Net.Cdan_wo_psi_fw(SDI, TDI)
            #print("finish Cdan1 forward")
            #Loss_DI = torch.mean(torch.relu(1.0 - DIPredict))
            #Loss_CDAE = torch.mean(torch.relu(1.0 + AEPredict))
            #L_Dpe = (Loss_DI + Loss_CDAE).to(device=device)
            #L_Cdan = Net.CdanLoss(DIPredict, AEPredict).to(device=device)
            #print("L_Dpe:{:.5f}".format(L_Dpe))
            ...
            (-SetParam.lada*(L_ASRec + L_ATRec)).backward(retain_graph=True)
        ...
        d_DI, d_CDAE, _, _, _, _ = Net.Cdanfw(d_SDI, d_TDI) #preparation for visualization
